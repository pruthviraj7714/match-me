...
export default function MemberCard({ member }: { member: UserProps }) {
  return (
    <Link href={`/members/${member.id}`} className="group relative h-60 w-60 overflow-hidden rounded-xl transition-all duration-300 ease-in-out hover:scale-105">
      ...
      <div className="absolute bottom-4 left-4 right-4 flex flex-col transition-all duration-300 ease-in-out group-hover:translate-y-[-4px]">
        <h3 className="text-xl font-bold text-white">{member.name}, {calculateAge(member.dateOfBirth)}</h3>
